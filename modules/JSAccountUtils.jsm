...
JSAccountUtils.jaFactory = function (aProperties, aJsDelegateConstructor)
{
  ...
  factory.createInstance = function(outer, iid)
  {
    ...
    if (delegateList instanceof Ci.msgIDelegateList) {
      ...
      log.info("creating delegate list for " + (aProperties.classDescription || aProperties.contractID));
...
{
  log.info("Making cppDelegator for " + (aProperties.contractID || aProperties.classDescription));
  let cppDelegator = {};
    ...
    if ('value' in descriptor) {
      ...
      property.value = function(aMethod) {
        return function(...args) {
          ...
          aProperties.baseInterfaces.forEach(iface => cppBase instanceof iface);
          return Reflect.apply(cppBase[aMethod], undefined, args);
        };
      }(method);
    }
    if (descriptor.set) {
...
// When you access a JA object as javascript rather than through
// XPCOM (as you might to access additional properties on the object,
// called here "extraInterfaces") there is a memory management issue.
//
// The JS object only has a weak reference to the delegator (weak because
// the delegator owns it, and C++ callers will see a reference count cycle
// if the ownership was strong). But if you instead hold onto a JS version of
// the object, then the delegator can get garbage collected away. The solution
// is to access wrappedJSObjects through a proxy object that holds a strong'
// reference to the delegator. This causes no problems in C++ because C++ never
// uses a wrapped JSObject.
//
// Because the extraInterfaces are only used in JS there is no reason to force
// these through XPCOM, just used the wrappedJSObject instead.
//
JSAccountUtils.strongJSObject = function(weakObject) {
  let strongPrototype = Object.create(weakObject);
  // Strong reference to the delegator, since the weakObject only has
  // a weak reference.
  strongPrototype._delegatorStrong = weakObject.delegator;

  // The proxy forces all methods to use the weak object rather
  // than the proxy of strongPrototype.
  let proxy = new Proxy(strongPrototype,
    {
      get(target, key, receiver) {
        // This has to work for functions as well as simple values.
        let getValue = weakObject[key];
        if ( (typeof getValue) == "function") {
          return function(...args) {
            return getValue.apply(weakObject, args);
          }
        }
        return getValue;
      },
      set(target, key, value) {
        weakObject[key] = value;
        return true;
      },
    });
  return proxy;
