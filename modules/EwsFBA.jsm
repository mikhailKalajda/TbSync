...
function doFbaExpanded(aUsername, aPassword, aUrl)
{
  ...
  tabProgressListener.onLocationChange = function onLocationChange(progress, request, location, flags)
  {
    ...
    if (tab)
      tab.browser.parentNode.appendNotification(location.spec, "", 'chrome://tbsync/skin/letter-x-icon-16.png', PRIORITY_INFO_LOW);
  };
  ...
  windowProgressListener.onLocationChange = function onLocationChange(progress, request, location, flags)
  {
    ...
    // show the current URL in the notification bar
    windowProgressListener.notifyBox.appendNotification(location.spec, "", 'chrome://tbsync/skin/letter-x-icon-16.png', PRIORITY_INFO_LOW);
  };
  ...
  if (isModal)
  {
    let fbaWindow = Services.ww.openWindow(activeWindow, "chrome://tbsync/content/moreinfo.xhtml", "ExQuillaFBA",
