...
var { EwsFBA } = ChromeUtils.import("resource://tbsync/EwsFBA.jsm");
...
EventListener.prototype = 
{
  ...
  {
    log.info('autodiscover handle event ' + event.type + ' readyState is ' + this.mRequest.readyState);
    try {
      if ((this.mChannelStatus = this.mRequest.channel.status))
        log.info("Channel status is " + CN(this.mChannelStatus));
    } catch (e) {}
    ...
    try {
      if (event.type == "load" || event.type == "error" || event.type == "abort" || event.type == "timeout")
      {
        ...
        { try {
          ...
          log.info("mEwsOWAGuessUrl is " + this.mEwsOWAGuessUrl);
