...
function openContentTab(aUrl, aStrRegEx, aDelay, aOnLoad, aOnListener, aSetTab)
{
  callLater( function _openTab()
  {
    ...
    let args =  { contentPage: aUrl,
                  background: false,
                  clickHandler: "tbsync.siteClickHandler(event, new RegExp(\"" + aStrRegEx + "\"));",
                };
...
function showStatusFeedbackString(aName, aParameter)
{
  ...
  try { // break on error
    ...
    if (aName) {
      let bundle = Services.strings.createBundle("chrome://tbsync/locale/tbsync.properties");
...
async function logErrorToServer(logMessage)
{
  try {
    ...
    const kExtPrefBranch = "extensions.tbsync.";
...
{
  let addon = await AddonManager.getAddonByID("exchange_sync@r7.ru");
...
function openAccountWizard(window) {
  try { /* COMPAT for TB 68 */
    ...
    Services.ww.openWindow(null, "chrome://tbsync/content/ewsAccountWizard.xul",
                "AccountWizard", "chrome,modal,titlebar,centerscreen", null);
  } catch (ex) { /* COMPAT for TB 68 */
    Services.ww.openWindow(null, "chrome://tbsync/content/ewsAccountWizard.xhtml",
                "AccountWizard", "chrome,modal,titlebar,centerscreen", null);
  } // COMPAT for TB 68
}
