...
exquilla.AW = (function exquillaAW()
{
  ...
  const exquillaStrings = Services.strings.createBundle("resource:///locale/exquilla.properties");
  ...
  {
    let alertText = aFoundAutodiscover ? "Найден сайт автообнаружения, но не удается найти подходящую запись. Возможно, это произошло потому, что ваш сервер не поддерживает Exchange Web Services (EWS). Если ваш сервер не поддерживает EWS, то работа плагина Р7 невозможна. Если это по вашему мнению произошло в результате ошибки, попытайтесь вручную ввести веб-адрес EWS."
                                       : "Невозможно найти сайт автообнаружения для вашего адреса электронной почты – он либо не существует, либо неправильно введено имя или пароль. Это может быть по причине отсутствия поддержки Exchange Web Services (EWS) вашим сервером. Если ваш сервер не поддерживает EWS, то работа плагина Р7 невозможна. Если это по вашему мнению произошло в результате ошибки, попытайтесь вручную ввести веб-адрес EWS.";
  ...
  function autodiscover(successCallback)
  {
    ...
    try {
      localScope = ChromeUtils.import("resource://tbsync/autodiscover.js");
    } catch(e) {
      log.warn(se(e));
      //try {re(e);} catch(e) {}
      // in testing, use the standard modules location
      try {
        localScope = ChromeUtils.import("resource://dummydiscover.js");
      } catch (e2) {
        log.warn(se(e2));
        throw e;
      }
  ...
  let adListener = {
    ...
    { try {
      ...
      if (aFoundSite)
      {
        ...
        _e("exquillaadresult").value = "Выполняется тест веб-адреса";
         if (aDisplayName) {
      ...
      else
      {
        ...
        if (aStatus < 1000) // html error
          resultString = "HTML# ";
        resultString += aStatus + " " + "Не выполнено";
      ...
      onEvent: function _onEvent(aItem, aEvent, aData, aResult)
      {
        ...
        if (aEvent == "StopMachine")
        {
          ...
          if (aResult == Cr.NS_OK)
          {
            ...
            _e("exquillaUrlResult").value = "Тест веб-адреса выполнен успешно";
          ...
          {
            _e("exquillaUrlResult").value = "Тест веб-адреса не выполнен";
...
;
            ...
            _e("exquillaUrlResult").value = "Выполняется тест веб-адреса";
