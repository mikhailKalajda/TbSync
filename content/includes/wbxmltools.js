...
// noinspection ES6ConvertVarToLetConst - used as var in network.js
var wbxmltools = {
    decode_mb_u_int32: function (buf, pos) {
        let out = { val: 0, len: 0 };
        let b;
        let limit = 100;
        do {
            b = buf.substr(pos + out.len, 1).charCodeAt(0);
            out.len += 1;
            limit--;
            out.val <<= 7;
            out.val += (b & 127);
        } while ((b & 128) !== 0 && limit > 0);

        if (limit <= 0) {
            TbSync.dump('decode_mb_u_int32 failed on bad input');
        }
    ...
    convert2xml: function (wbxml) {
        ...
        let xml = "";
        let codepage = 0;
        let noLoop = wbxml.length + 100;

        while (num < wbxml.length && noLoop > 0) {
            noLoop--;
            ...
            if (data === 195) {
                ...
                let len = this.decode_mb_u_int32(wbxml, num + 1);
                TbSync.dump("Opaque len = " + JSON.stringify(len));
                let opaqueEnd = num + len.len + 1 + len.val;
                let opaque = wbxml.substring(num + len.len + 1, opaqueEnd);
                TbSync.dump("Opaque = " + opaque);
                xml = xml + opaque;
                num = opaqueEnd;
            } else {
                switch (token) {
                    ...
                    case 0xC3: // Opaque document-type-specific data.
                        throw new Error('Must be handled separately')
                        break;
    ...
    createWBXML: function (wbxmlstring = String.fromCharCode(0x03, 0x01, 0x6A, 0x00), initialCodepage = "") {
        let wbxml = {
            ...
            singleTag: function (tokenname) {
                //check if tokenName is in current codepage
                if (!(this._codepage in wbxmltools.codepages2)) {
                    throw "[wbxmltools] Unknown codepage <" + this._codepage + ">";
                }
                if (!(tokenname in wbxmltools.codepages2[this._codepage])) {
                    throw "[wbxmltools] Unknown tokenname <" + tokenname + "> for codepage <" + wbxmltools.namespaces[this._codepage] + ">";
                }

                this._wbxml += String.fromCharCode((wbxmltools.codepages2[this._codepage][tokenname]));
            },

            encode_u_int32_mb: function (value) {
                let byteList = [];

                while (value > 0)
                {
                    let addByte = value & 0x7F;
            ...
            // adding a opaque string content tag as <tagname>contentstring</tagname>
            btag: function (tokenname, content = "") {
                //check if tokenName is in current codepage
                if (!(this._codepage in wbxmltools.codepages2)) {
                    throw "[wbxmltools] Unknown codepage <" + this._codepage + ">";
                }
                ...
                let len = this.encode_u_int32_mb(content.length);

                this._wbxml += String.fromCharCode((wbxmltools.codepages2[this._codepage][tokenname]) | 0x40);
                this._wbxml += String.fromCharCode(195);

                for (let i = 0; i < len.length; i++) {
                    let charCode = String.fromCharCode(len[i]);
                    TbSync.dump('btag code = ' + len[i] + ', charCode = "' + charCode + '", ' + charCode.charCodeAt(0));
                    this._wbxml += charCode;
                }

                for (let i = 0; i < content.length; i++) {
                    this._wbxml += String.fromCharCode(content.charCodeAt(i));
                }

                this._wbxml += String.fromCharCode(0x01); // close opaque
                this._wbxml += String.fromCharCode(0x01); // close token
            },
