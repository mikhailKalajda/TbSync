...
const {Utils} = ChromeUtils.import("resource://tbsync/ewsUtils.jsm");
...
var Calendar = {
    ...
    UpdateAttendees: function (item, data, syncdata) {
        ...
        let itemAttendees = item.getAttendees(itemAttendeesCount);

        TbSync.dump('UpdateAttendees out: itemAttendeesCount=' + JSON.stringify(itemAttendeesCount) + ', attendees=' + JSON.stringify(itemAttendees));
    ...
    setThunderbirdItemFromWbxml: function (tbItem, data, id, syncdata, mode = "standard") {
        ...
        if (TbSync.prefs.getIntPref("log.userdatalevel") > 2) {
            TbSync.dump("Processing " + mode + " calendar item", item.title + " (" + id + ")");
        }
        ...
        this.UpdateAttendees(item, data, syncdata);
    ...
    UpdateAttendeesIn: function (item, wbxml, asversion, isException, syncdata) {
        TbSync.dump('UpdateAttendees in started');
        ...
        if (!isException) { //Exchange 2010 does not seem to support MeetingStatus at all in exceptions
            if (countAttendees.value === 0) {
                wbxml.atag("MeetingStatus", "0");
            } else {
                //get owner information
                let isReceived = item.hasProperty("X-EAS-MEETINGSTATUS") ?
                    item.getProperty("X-EAS-MEETINGSTATUS") & 0x2 :
                    (item.organizer && item.organizer.id && cal.email.removeMailTo(item.organizer.id) !== syncdata.accountData.getAccountProperty("user"));
            ...
            if (countAttendees.value > 0) {
                ...
                for (let attendee of attendees) {
                    ...
                    if (asversion !== "2.5") {
                        ...
                        if (attendee.userType === "RESOURCE" || attendee.userType === "ROOM" || attendee.role === "NON-PARTICIPANT") {
                            wbxml.atag("AttendeeType", "3");
                        }
                        else if (attendee.role === "REQ-PARTICIPANT" || attendee.role === "CHAIR") {
                            wbxml.atag("AttendeeType", "1");
                        }
                        else {
                            wbxml.atag("AttendeeType", "2");
                        } //leftovers are optional
                    }
                    wbxml.ctag();
                }
                wbxml.ctag();
    ...
    getWbxmlFromThunderbirdItem: function (tbItem, syncdata, isException = false) {
        ...
        if (!isException) {
            ...
            let tzInfo = null;
            if (item.startDate && item.startDate.timezone.tzid !== "floating") {
                tzInfo = eas.tools.getTimezoneInfo(item.startDate.timezone);
            }
            else if (item.endDate && item.endDate.timezone.tzid !== "floating") {
                tzInfo = eas.tools.getTimezoneInfo(item.endDate.timezone);
            }
            if (!tzInfo) {
                tzInfo = eas.defaultTimezoneInfo;
            }
