...
EwsService.prototype = {
  ...
  addAbFromNativeContactFolder(aNativeFolder)
  {
    ...
    if (!matchingDirectory && (!isDeleted) && useAB)
    {
      ...
      // TODO: let returnedURI = MailServices.ab.newAddressBook(displayName, uri, MAPIDirectory, dirPref);

      // TODO: matchingDirectory = MailServices.ab.getDirectory(uri);
      // TODO: let ewsDirectory = safeGetJS(matchingDirectory);

      // glue between the native folder and the ab directory
      // TODO: ewsDirectory.folderId = folderId;
      // TODO: if (aNativeFolder.distinguishedFolderId)
      // TODO:   ewsDirectory.distinguishedFolderId = aNativeFolder.distinguishedFolderId;
    }

    else if (matchingDirectory && (isDeleted || !useAB))
    {
      // deleted from the address book manager
      // TODO: MailServices.ab.deleteAddressBook(matchingDirectory.URI);
    }
    ...
    if (subfolderIds)
    {
      for (let ii = 0; ii < subfolderIds.length; ii++)
      {
        ...
        // TODO: this.addAbFromNativeContactFolder(subNativeFolder);
      }
    }
    return matchingDirectory;
  },
